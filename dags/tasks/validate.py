        ...
        for stream_key in new_files:
            try:
                ...
                if FAIL_ON_VALIDATION_ERROR:
                    ...
                    logger.warning(f"Continuing with other files due to resilient mode setting.")
                    continue
        
        # If we're in resilient mode and have failures, still report them
        if failed_files and not FAIL_ON_VALIDATION_ERROR:
            logger.warning(f"{len(failed_files)} files failed validation but pipeline continued: {failed_files}")
